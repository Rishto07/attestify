...
- **Git pre-commit hook**: `verdict hook install` writes a no-dependency POSIX hook; `verdict diff-check` scans staged changes and blocks on danger. Heart-shots proven live: a staged `curl | bash` was blocked at commit; a safe change committed cleanly. The hook even flagged its own feature commit, exposing and fixing a `>/dev/null` false positive (`/dev/null`, `/dev/stderr` are now safe; `/dev/tcp` exfil and real system writes still flag)