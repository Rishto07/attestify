...
``verdict hook install`` copies a dependency-free POSIX-sh hook into
``.git/hooks/pre-commit``. The hook runs ``verdict diff-check`` against your
...
# Installed by `verdict hook install`. Bypass on purpose: git commit --no-verify
if command -v verdict >/dev/null 2>&1; then
    VERDICT_CMD="verdict"
elif command -v python  >/dev/null 2>&1; then
    VERDICT_CMD="python -m verdict"
elif command -v python3 >/dev/null 2>&1; then
    VERDICT_CMD="python3 -m verdict"
else
    echo "verdict: not found; skipping pre-commit check" >&2
    exit 0
fi

if [ "${VERDICT_SKIP:-}" = "1" ]; then
    echo "verdict: skipped via VERDICT_SKIP=1" >&2
    exit 0
fi
...
if [ "$RC" -eq 2 ]; then
    echo "$FOUND"
    echo "verdict: BLOCKED - dangerous pattern in staged changes."
...
def install(start: Optional[Path] = None) -> bool:
    ...
    if target.exists():
        ...
        if "Verdict pre-commit hook" in existing:
            print(f"verdict: hook already installed at {target}")
            return True
        ...
        print(f"verdict: {target} exists and is not a Verdict hook; leaving it alone")
    ...
    try:
        ...
        pass
    print(f"verdict: pre-commit hook installed -> {target}")
...
def uninstall(start: Optional[Path] = None) -> bool:
    ...
    if not target.exists():
        print("verdict: no hook installed")
        return True
    text = target.read_text(encoding="utf-8", errors="ignore")
    if "Verdict pre-commit hook" not in text:
        print(f"verdict: {target} is not a Verdict hook; not removing it")
        return False
    target.unlink()
    print(f"verdict: removed {target}")
    return True
...
def status(start: Optional[Path] = None) -> int:
    ...
    hd = hooks_dir(start)
    if hd is None:
        print("verdict: not a git repository")
        return 2
    target = hd / HOOK_NAME
    if not target.exists():
        print("verdict: no pre-commit hook installed")
        return 1
    text = target.read_text(encoding="utf-8", errors="ignore")
    if "Verdict pre-commit hook" in text:
        print(f"verdict: pre-commit hook installed -> {target}")
        return 0
    print(f"verdict: a non-Verdict pre-commit hook exists at {target}")
    return 2