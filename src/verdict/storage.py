...
Every verdict produces a receipt. This module stores them in a simple way:
...
class ReceiptStore:
    ...
    def __init__(self, path: str | Path | None = None):
        """
        Args:
            path: Directory to store receipts. Defaults to ./verdict-data in cwd,
                  or VERDICT_DATA_DIR env var.
        """
        if path is None:
            path = os.environ.get("VERDICT_DATA_DIR", "verdict-data")
        self.path = Path(path)
    ...
    def query(
        ...
        """Query receipts by date range and/or verdict.

        Args:
            start_date: Start date (YYYY-MM-DD), inclusive
            end_date: End date (YYYY-MM-DD), inclusive
            verdict: Filter by "PASS", "FAIL", or "UNKNOWN"
    ...
    def stats(self) -> dict:
        ...
        total_receipts = 0
        by_verdict = {"PASS": 0, "FAIL": 0, "UNKNOWN": 0}
