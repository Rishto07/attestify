...
def block_attestify() -> Verdict:
    """Aggregate the diff findings into a checkable attestify.
...
def _run_diffcheck(extra_args: Optional[list[str]] = None) -> int:
    """CLI body for `attestify diff-check`. Returns process exit code."""
    try:
        result = scan_staged_changes(extra_args)
    except GitError as e:
        print(f"attestify diff-check: {e}", flush=True)
    ...
    if not result.dangerous:
        ...
        print(f"[PASS] attestify: no dangerous patterns in staged changes{scan_note}", flush=True)
        return 0

    print(f"[FAIL] attestify: {len(result.findings)} dangerous pattern(s) in staged changes", flush=True)
