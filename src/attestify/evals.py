...
class EvalDataset:
    ...
    positive: str  # which attestify value is the "positive" class (e.g. "FAIL")
...
def main(argv: Optional[list[str]] = None) -> int:
    """Entry point for `attestify evals`.

    Usage:
        attestify evals                # offline checkers only (quarantine + execute)
        attestify evals --prosecutor   # + judge-reliability eval (needs LLM in .env)
        attestify evals --json         # machine-readable report
    """
    import argparse

    parser = argparse.ArgumentParser(prog="attestify evals", description="Run Verdict's evaluation corpus.")
