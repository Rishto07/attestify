...
- ``SubprocessSandbox`` — runs code as the current user (the old behavior).
    ...
    and always reported as such in the attestify.
...
class DockerSandbox(Sandbox):
    ...
    def run(
        ...
        cmd = [
            ...
            "--name",
            f"attestify-sbx-{int(time.time() * 1000)}",
            image,
            "timeout",
            f"{inner}s",
        ] + argv
...
def get_sandbox(kind: str | None = None) -> Sandbox:
    ...
    Resolution order for ``kind=None`` (and ``ATTESTIFY_SANDBOX`` unset):
    1. docker is available -> DockerSandbox (isolated)
    2. otherwise            -> SubprocessSandbox (explicit fallback)
    """
    if kind is None:
        kind = os.environ.get("ATTESTIFY_SANDBOX", "")
