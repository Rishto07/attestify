...
- ``Receipt`` — an immutable, hash-locked record of a attestify. You can trust
...
class Verdict:
    ...
    - ``FAIL`` wins: any evidence with ``weight >= fail_threshold`` that
      concluded FAIL makes the whole attestify FAIL.
...
@dataclass(frozen=True)
class Receipt:
    """An immutable, hash-locked record of one attestify.
    ...
    replay the exact evidence a attestify rested on. That is the point: Verdict
    ...
    def create(
        ...
        payload: dict[str, Any] = {
            ...
            "pipeline": pipeline,
            "attestify": attestify_obj.value.value,
            "confidence": attestify_obj.confidence,
            "summary": attestify_obj.summary,
            "evidence": [e.to_dict() for e in attestify_obj.evidence],
            "model": model,
...
class CheckContext:
    ...
    def __init__(
        ...
        # A Sandbox instance (attestify.sandbox). Kept as Any to avoid a hard
...
class Checker(abc.ABC):
    ...
    this checker's FAIL counts against the overall attestify.
    """
