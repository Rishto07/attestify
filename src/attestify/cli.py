"""Attestify CLI — the trust layer for AI output.

Usage:
    attestify check <output>       # Run all checkers on output
    attestify check --file <path>  # Run on file contents
    attestify quarantine <output>  # Just the quarantine scanner
    attestify execute <output>     # Just the ExecuteProof checker
    attestify receipt <id>         # Show a specific receipt
    attestify stats                # Show storage statistics

Environment:
    ATTESTIFY_LLM_URL      OpenAI-compatible endpoint
    ATTESTIFY_LLM_KEY      API key
    ATTESTIFY_LLM_MODEL    Model to use for prosecutor (default: gpt-4o-mini)
    ATTESTIFY_DATA_DIR     Where to store receipts (default: ./attestify-data)
"""
